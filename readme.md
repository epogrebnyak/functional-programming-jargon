...
с примерами на Нaskell. Оригинальная статья использует примеры на JavaScript, она переведена на русский [здесь](https://habr.com/ru/post/310172).
...
## Что поменялось в переводе по сравнению с исходной статьей? <!-- omit in TOC -->
...
Я добавил статьи *Функция* и *Тип*, которые открывают соответствующие разделы.
...
программированию (ФП), а Haskell и есть сам ФП.

__Ссылки__

В отдельных статьях добавлены ссылки на [глоссарий haskell.org][hg], 
полезные вопросы и ответы StackOverflow. В конце словаря
приводится небольшой список литературы.
...
Вы можете дать комментарий в [issues этого проекта](https://github.com/epogrebnyak/functional-programming-jargon/issues), или написать переводчику.
...
inc :: Num a => a -> a
```
Эта подпись, выведенная компилятором, говорит о том, 
