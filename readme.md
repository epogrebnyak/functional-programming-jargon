...
__Примечание:__ исходя из более строгой терминологии, принятой в [лямбда-исчислении](#лямбда-исчисление), широко распространенные *"лямбда"* и *"лямбда-функция"* - это не совсем точные, сленговые выражения. 

> Для того, чтобы определить функцию, не обязательно задавать её имя. Для этого можно воспользоваться [лямбда-абстракцией](https://wiki.haskell.org/Lambda_abstraction) `\x -> x + 1`. Такие функции будем называть анонимными. 
